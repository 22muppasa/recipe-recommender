...
class RecipeDataStreamer:
    ...
    def load_full_dataset(self):
        """Load the complete dataset in chunks and process efficiently"""
        logger.info("🚀 Starting to load full dataset...")
        ...
        try:
            ...
            logger.info("📊 Reading complete dataset...")
            self.recipes_df = pd.read_csv(self.data_file_path)
            
            initial_count = len(self.recipes_df)
            logger.info(f"📊 Loaded {initial_count} total recipes")
            
            # Filter out recipes with missing essential data
            self.recipes_df = self.recipes_df.dropna(subset=['Name', 'RecipeIngredientParts'])
            filtered_count = len(self.recipes_df)
            
            logger.info(f"📊 After filtering: {filtered_count} recipes with complete data")
    ...
    def prepare_ml_data(self):
        """Prepare ML data by processing ingredients in chunks"""
        ...
        try:
            # Process ingredients in chunks to handle large dataset
            ingredients_text = []
            valid_indices = []
            ...
            for chunk_start in range(0, total_recipes, self.chunk_size):
                chunk_end = min(chunk_start + self.chunk_size, total_recipes)
                chunk = self.recipes_df.iloc[chunk_start:chunk_end]
                
                logger.info(f"🔄 Processing chunk {chunk_start}-{chunk_end} ({processed}/{total_recipes})")
                    ...
                    try:
                        ...
                        if ingredients:
                            ...
                            if clean_ingredients:
                                ...
                                valid_indices.append(chunk_start + idx)
                    except Exception as e:
            ...
            # Create TF-IDF vectorizer with optimized parameters for large dataset
    ...
    def search_recipes(self, search_ingredients, top_n=6):
        ...
        try:
            ...
            # Get top matches
            top_indices = similarities.argsort()[-top_n*3:][::-1]  # Get more candidates
