...
from data_streamer import RecipeDataStreamer
...
def initialize_data_streamer():
    """Initialize the data streamer with the full dataset"""
    ...
    try:
        ...
        logger.info(f"🚀 Initializing data streamer with file: {full_file}")
        
        if not os.path.exists(full_file):
            logger.error(f"❌ Dataset file not found: {full_file}")
            return False
        
        data_streamer = RecipeDataStreamer(full_file)
        
        # Load the dataset
        if not data_streamer.load_full_dataset():
        ...
        logger.info("✅ Data streamer initialized successfully")
        return True
...
logger.info("🚀 Starting recipe backend initialization...")
if initialize_data_streamer():
    logger.info("✅ Recipe backend ready!")
else:
